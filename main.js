...
// 🔇 كتم كل الأصوات (طلب المستخدم). لو تريد ترجع صوت وحد معين بس، غيّر
// السطر المطابق إله تحت من 0 إلى 1 (مثلاً SFX_FLAP.volume = 1 لصوت الطيران بس).
SFX_SCORE.volume = 0
SFX_FLAP.volume = 0
SFX_COLLISION.volume = 0
SFX_FALL.volume = 0
SFX_SWOOSH.volume = 0
...
/*************************************************************
***** تكامل تيليجرام Web App — إرسال السكور تلقائياً للبوت *****
*************************************************************/
// ملاحظة: هذا القسم كامل مضاف بالآخر بس، ما يلمس ولا سطر من كود اللعبة
// الأصلي فوق — نفس التصميم والحركة والصوت 100% زي ما هي.
(function () {
    // يقرأ بيانات الجلسة (chat_id, user_id, token) اللي البوت حطها بالرابط
    function getSessionData() {
        try {
            const params = new URLSearchParams(window.location.search)
            const encoded = params.get('d')
            if (!encoded) return null
            // نفس ترميز base64 urlsafe اللي البوت يستخدمه (بدون padding)
            let b64 = encoded.replace(/-/g, '+').replace(/_/g, '/')
            while (b64.length % 4) b64 += '='
            return JSON.parse(decodeURIComponent(escape(window.atob(b64))))
        } catch (e) {
            console.error('فشل قراءة بيانات الجلسة:', e)
            return null
        }
    }
    ...
    // نراقب حالة اللعبة كل 200ms، ولما توصل "Game Over" نرسل السكور مرة
    // وحدة بس (scoreSent يمنع الإرسال المتكرر وهي واقفة على شاشة الخسارة)
    let scoreSent = false
    setInterval(function () {
        if (typeof gameState === 'undefined' || typeof score === 'undefined') return
        ...
        if (gameState.current === gameState.gameOver) {
            if (!scoreSent) {
                ...
                if (tg && session) {
                    ...
                    // sendData تلقائياً تسكر صفحة الـ Web App بعد الإرسال
                } else {
                    console.warn('ماكو Telegram WebApp أو جلسة صالحة — السكور ما راح ينرسل.')
                }
            }
        } else {
            // رجعت اللعبة تلعب من جديد — نسمح بإرسال سكور جديد بالجولة الجاية
            scoreSent = false
        }
    }, 200)
})()